...
unsigned char	SIX_LED_display_time;
unsigned char	SIX_LED_Cycle;
unsigned char 	SIX_LED_KEY_LONG_PRESS_CNT;
...
#define TH1_INIT        (65536-267)/256 //2.5ms@XTAL=12MHz, Period = (5.425/2) ms@XTAL=22.1184MHz
#define TL1_INIT        (65536-267)%256
...
unsigned char get_SIX_LED_Key(void)
{
	...
	if (P05 == 0)
	{
		...
		if (SIX_LED_KEY_debounce > SIX_LED_KEY_LONG_PRESS_CNT)
		{
...
void setPWM_BUZZRE(unsigned char status)
{
	if (status == 0)
	{
	...
	else
	{
		...
		PWM_CLOCK_FSYS;
		PWMPH = 0x19;
		PWMPL = 0x09;	//2.4K=sys/(pwmph,pwmpl)
		set_SFRPAGE;
		PWM0H = 0x12;
		PWM0L = 0x04;
		clr_SFRPAGE;
...
void RF_CK_Process(unsigned char valuetemp)
{
	if (valuetemp& 0x01)
	{
		/*	short key	*/
...
void main(void)
{
	...
	SIX_LED_display_time = 0;
	SIX_LED_KEY_LONG_PRESS_CNT = 200;
	RF_Level_add_cnt = 0;
	RF_Level_del_cnt = 0;
    while (1)
	{
		if (is_5ms_Flag)
		{
			...
			1: Normal on
			2: Normal off
			0x81: 1HZ flash on;0x91:1HZ flash off
			0x82: 2hz flash	on;0x92:2HZ flash off
			0x83: 3hz flash	on;0x93:3HZ flash off
			0x85: 5hz flash	on;0x95:5HZ flash off
			0x87: 7hz flash	on;0x97:7HZ flash off
			0x8A: 10hz flash on;0x9A:10HZ flash off
			*/
			if (valuetemp == 1)
			{
				if (SIX_LED_ON_OFF == OFF)
				{
					...
					else if (SIX_LED_Freq & 0x80)
					{
						SIX_LED_display_time = 100 / (SIX_LED_Freq & 0x0F);
...
//						SIX_LED_ON_OFF = OFF;
						isSIX_LED_ENTER_FLASH_MODE = 0;
						P11 = 0;
						SIX_LED_KEY_LONG_PRESS_CNT = 100;
					}
//					else if (SIX_LED_Freq == 0x80)
//					{
//						SIX_LED_Freq = 0x81;
...
//						SIX_LED_KEY_LONG_PRESS_CNT = 200;
//					}
					else if ((SIX_LED_Freq & 0x8F) == 0x80)
					{
						SIX_LED_Freq = 0x82;
						SIX_LED_ON_OFF = ON;
						isSIX_LED_ENTER_FLASH_MODE = 1;
					...
					else if ((SIX_LED_Freq & 0x8F) == 0x82)
					{
						SIX_LED_Freq = 0x83;
						SIX_LED_ON_OFF = ON;
						isSIX_LED_ENTER_FLASH_MODE = 1;
					...
					else if ((SIX_LED_Freq & 0x8F) == 0x83)
					{
						SIX_LED_Freq = 0x85;
						SIX_LED_ON_OFF = ON;
						isSIX_LED_ENTER_FLASH_MODE = 1;
					...
					else if ((SIX_LED_Freq & 0x8F) == 0x85)
					{
						SIX_LED_Freq = 0x87;
						SIX_LED_ON_OFF = ON;
						isSIX_LED_ENTER_FLASH_MODE = 1;
					...
					else if ((SIX_LED_Freq & 0x8F) == 0x87)
					{
						SIX_LED_Freq = 0x8A;
						SIX_LED_ON_OFF = ON;
						isSIX_LED_ENTER_FLASH_MODE = 1;
					...
					else if ((SIX_LED_Freq & 0x8F) == 0x8A)
					{
						SIX_LED_Freq = 1;
						SIX_LED_ON_OFF = ON;
						isSIX_LED_ENTER_FLASH_MODE = 0;
			...
			if (SIX_LED_ON_OFF == ON)
			{
				...
				{
					if (SIX_LED_display_time > 0)
						SIX_LED_display_time--;
					if (SIX_LED_display_time == 0)
					{
						P11^=1;
						SIX_LED_display_time = 100 / (SIX_LED_Freq & 0x0F);
